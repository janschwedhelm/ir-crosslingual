...
def vec2features(sens: Sentences, dim: int):
    unique_queries = sens.test_collection.drop_duplicates(subset=['src_sentence'], ignore_index=True)
    print('---- INFO: Unique queries extracted')
    unique_documents = sens.test_collection.drop_duplicates(subset=['trg_sentence'], ignore_index=True)
